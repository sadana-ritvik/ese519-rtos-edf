...
// Returns the node for this task ID, or NULL if it doesn't
// exist in the ready queue.
nrk_queue *nrk_get_node_in_readyQ(int8_t task_ID) {
	  nrk_queue *cur = _head_node;
    while (cur != NULL) {
...
// Returns true if this task ID is present in the ready queue,
// and false if it is not.
bool nrk_present_in_readyQ(int8_t task_ID) {
	  return nrk_get_node_in_readyQ(task_ID) != NULL;
}
...
void nrk_add_to_readyQ(int8_t task_ID) {
    ...
    CurNode = _free_node;
		...
		// Find the node to insert at, based on task_ID's priority.
    if (_head_node != NULL) {
        while (NextNode != NULL) {
		...
		// TODO: handle NextNode being NULL? (
...
void nrk_rem_from_readyQ(int8_t task_ID) {
    ...
    if (_head_node->task_ID == task_ID) {
        ...
        if (CurNode == NULL) {
					  // end of list and task not present. 
...
//nrk_cur_task_TCB->cpu_remaining=_nrk_prev_timer_val+1;
    ...
    if (timer < (MAX_SCHED_WAKEUP_TIME - TIME_PAD)) // if(timer<(250-2))
